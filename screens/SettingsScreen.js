...
export default function SettingsScreen({ navigation, route }) {
  ...
  const [userEmail, setUserEmail] = useState("OSA@slipin.app");
  const [userTags, setUserTags] = useState("Creative · Tech");
  const [userBio, setUserBio] = useState("Hey there! I'm using SlipIn."); // Bio add kar di
  ...
  useEffect(() => {
    ...
    // --- Profile Update Logic from EditProfileScreen ---
    if (route.params?.updatedProfile) {
      const { name, email, tags, image, bio } = route.params.updatedProfile;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {/* Profile Card - Ab yeh EditProfileScreen par jaye ga */}
        <TouchableOpacity 
        ...
        {/* Baki sections (Visibility, Location, Discovery etc.) same rahain ge */}
        <View style={styles.section}>
          ...
          <View style={styles.sectionCard}>
            ...
            />
            <CustomDivider />
            <SettingRow icon="🌑" label="Dark Zones" value="Manage" onPress={() => navigation.navigate("DarkZones")} />
            <CustomDivider />
            <SettingRow icon="👻" label="Ghost Delay" value="15 min" onPress={() => navigation.navigate("GhostDelay")} />
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.sectionCard}>
            <SettingRow icon="📍" label="Radius Level" value={currentRadius} onPress={() => navigation.navigate("VisibilityRadius", { currentRadius })} />
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.sectionCard}>
            <SettingRow icon="🎯" label="Filters" value={activeFilters} onPress={() => navigation.navigate("FilterModal", { activeFilters })} />
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  avatarWrapper: { position: 'relative', width: 60, height: 60 },
  avatarContainer: { width: 60, height: 60, borderRadius: 30, overflow: 'hidden', backgroundColor: '#222' },
  profileImg: { width: 60, height: 60, resizeMode: 'cover' },
  onlineDot: { position: 'absolute', bottom: 2, right: 2, width: 14, height: 14, borderRadius: 7, backgroundColor: '#00FF9D', borderWidth: 2, borderColor: '#1A1A24' },
  ...
  editBtn: { paddingVertical: 4, paddingHorizontal: 12, borderRadius: 20, backgroundColor: 'rgba(0, 255, 157, 0.1)' },
  editText: { fontSize: 12, color: "#00FF9D", fontWeight: "600" },
  section: { paddingHorizontal: 16, marginBottom: 20 },
  sectionTitle: { color: "#888888", fontSize: 12, fontWeight: "700", letterSpacing: 1.2, marginBottom: 8, marginLeft: 4 },
  sectionCard: { backgroundColor: "transparent", overflow: "hidden" },
  ...
  logoutBtn: { marginHorizontal: 16, marginVertical: 8, paddingVertical: 14, borderRadius: 12, borderWidth: 1, borderColor: "#FF4444", alignItems: "center" },
