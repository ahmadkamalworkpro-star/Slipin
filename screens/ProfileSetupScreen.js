...
import * as ImagePicker from "expo-image-picker"; // Gallery access ke liye
...
export default function ProfileSetupScreen({ navigation, route }) {
  // SettingsScreen se aane wala data (if any)
  const { currentName, currentEmail, currentTags, currentAge, currentGender, currentBio } = route.params || {};
  ...
  const pickImage = async () => {
    ...
    if (status !== "granted") {
      alert("Hume gallery permission chahiye photo upload karne ke liye!");
      return;
    }
  ...
  const handleContinue = () => {
    if (step === 1) {
      ...
      // Final Save: Wapis Settings ko data bhej rahe hain
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <NeonButton
          title={step === 1 ? "Continue →" : "Save Changes"}
